...
$('.play-button').on('click', (e) => {
  ...
  if (!playing) {
    ...
    audio = new Audio(`https://live.upbeat.pw?cacheBuster=${Math.ceil(Math.random() * 40000)}`);
    audio.volume = volume;
    playing = true;
    audio.play();
  } else {
...
const fetchSongData = () => {
  ...
  .then(({ song }) => {
    ...
    $('.song-art:not(.dj-art)').attr('src', song.graphic.medium || "https://rogroup.github.io/images/FRESH%20NEW.png")
    $('.song-background img').attr('src', song.graphic.medium || "https://rogroup.github.io/images/FRESH%20NEW.png")
  });
};
...
const fetchSlotData = () => {
  ...
  .then((result) => {
    ...
    $('.dj-background img').attr('src', result.user.avatar || "https://rogroup.github.io/images/FRESH%20NEW.png");
    ...
    $('.presenter-live .presenter-image').attr('src', result.user.avatar || "https://rogroup.github.io/images/FRESH%20NEW.png");
  })
  .catch(() => {
    ...
    $('.presenter-live .presenter-image').attr('src', 'https://rogroup.github.io/images/FRESH%20NEW.png');
  });
  ...
  .then((result) => {
    ...
    $('.presenter-next .presenter-image').attr('src', result.user.avatar || "https://rogroup.github.io/images/FRESH%20NEW.png");
  })
  .catch(() => {
    $('.presenter-next .presenter-name').text('Fresh Stream');
    $('.presenter-next .presenter-time').text("");
    $('.presenter-next .presenter-image').attr('src', 'https://rogroup.github.io/images/FRESH%20NEW.png');
  });
  ...
  .then((result) => {
    ...
    $('.presenter-later .presenter-image').attr('src', result.user.avatar || "https://rogroup.github.io/images/FRESH%20NEW.png");
  })
  .catch(() => {
    $('.presenter-later .presenter-name').text('Fresh Stream');
    $('.presenter-later .presenter-time').text("");
    $('.presenter-later .presenter-image').attr('src', 'https://rogroup.github.io/images/FRESH%20NEW.png');
  });
      ...
      _embedded.forEach((songPlay) => {
        $('.song-history-container').append(`
          <div class="song-history-item">
            <img src="${(songPlay.song.graphic && songPlay.song.graphic.large) || "https://rogroup.github.io/images/FRESH%20NEW.png"}" alt="" class="song-history-item-image">
